...
#        vals = model.cbGetSolution(model._vars)
        ...
        #selected_clusters = gp.tuplelist((i,k) for (i,k), var_ in model._vars.items() if var_.y > 0.5)
        ...
        for subtour in tours:
            if len(subtour) == len(M):  # full tour across all clusters — valid, skip
                continue
            # print(subtour)
            # subtour = tours[0]
...
def _prepare_instance(inst):
    ...
    N = len(inst['x_coords'])
    C = inst['cluster_count']
    coords = np.column_stack((inst['x_coords'], inst['y_coords']))
    #len(coords)
    clusters = get_clusters(np.array(inst['clusters']), C)
...
n_i = dict(Counter(sample["clusters"]))
...
model.write(f"ds502_project.lp")  # Human-readable LP format
...
if model.Status in [GRB.OPTIMAL, GRB.TIME_LIMIT] and model.SolCount > 0:
    ...
    result['visited_nodes'] = list(set(visited))  # Remove duplicates from multi-vehicle
