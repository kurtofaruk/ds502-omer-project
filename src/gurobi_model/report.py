...
report_df=pd.read_excel("/Users/kurtmac/Desktop/git_files/ds502-omer-project/reports/results_extension_TSP.xlsx",index_col=0)
...
any_nonzero = any(g > 0 for g in gap_pct)
if any_nonzero:
    ...
    ax.set_ylim(0, 1)
    ax.text(0.5, 0.5,
            'All instances solved to optimality\n(gap = 0, abs_gap = 0)',
            transform=ax.transAxes, ha='center', va='center',
            fontsize=10, color='#1D9E75', fontstyle='italic')
...
# ── 3. Gap analysis ───────────────────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(12, 8))
any_nonzero = any(g > 0 for g in gap) or any(g > 0 for g in abs_gap)
if any_nonzero:
